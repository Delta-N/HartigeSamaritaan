...
export class Validator {
	static date(control: FormControl): { [p: string]: any } | null {
		...
		if (!control.value.match(datePattern)) return { date: true };
	...
	static phoneNumber(control: FormControl): { [p: string]: any } | null {
		...
		if (!control.value.match(numberPattern)) return { phoneNumber: true };
	...
	static postalCode(control: FormControl): { [p: string]: any } | null {
		...
		if (!control.value.match(postalCodePattern)) return { postalCode: true };
	...
	static email(control: FormControl): { [p: string]: any } | null {
		...
		if (!control.value.match(emailPattern)) return { email: true };
