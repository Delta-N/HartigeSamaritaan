...
export class ProjectComponent implements OnInit {
	...
	async getParticipation() {
		await this.participationService
			.getParticipation(this.userService.getCurrentUserId(), this.guid)
			.then(async (res) => {
				if (res) {
	...
	displayProject(project: Project | null) {
		...
		this.viewProject.closed
			? (this.closeButtonText = 'Project openen')
			: (this.closeButtonText = 'Project afsluiten');
	...
	async closeProject() {
		...
		this.project.closed
			? (messageVariable = 'openen')
			: (messageVariable = 'sluiten');
		const message =
			'Weet je zeker dat je dit project wilt ' + messageVariable + ' ?';
	...
	editWorkingHours() {
		const message =
			'Hoeveel uur per week wil je maximaal meewerken aan dit project?';
		const dialogData = new ConfirmDialogModel(
			'Maximale inzet ',
			message,
			'NumberInput',
			this.participation.maxWorkingHoursPerWeek
		);
		const dialogRef = this.dialog.open(ConfirmDialogComponent, {
			maxWidth: '400px',
			data: dialogData,
		...
		dialogRef.afterClosed().subscribe(async (dialogResult) => {
			...
			if (
				...
				await this.participationService
					.updateParticipation(this.participation)
					.then(async (response) => {
						if (response) {
	...
	expandTaskCard() {
		...
		if (this.taskCardStyle === 'expanded-card') {
			if (leftElement) leftElement.hidden = false;
			...
			if (pictureElement) pictureElement.hidden = true;
	...
	async exportShifts() {
		...
		let guid: string;
		if (this.participation) guid = this.participation.project.id;
		else guid = this.project.id;
		this.loaded = false;
		await this.shiftService.GetExportableData(guid).then((res) => {
			...
			res?.forEach((shift) => {
				shift.availabilities.forEach((avail) => {
					const statistic = {
						Taaknaam: shift.task
							? shift.task.name.replace(',', '.')
							: 'Onbekend',
						Taakcategorie:
							shift.task && shift.task.category
								? shift.task.category.name.replace(',', '.')
								: 'Onbekend',
						...
						NaamMedewerker:
							avail.participation.person.firstName.replace(',', '.') +
							' ' +
							avail.participation.person.lastName.replace(',', '.'),
			...
			this.csvService.downloadFile(
				statistics,
				headers,
				'Shift export - ' + this.project.name
			);
		});
		this.loaded = true;
	}
	...
	async exportUsers() {
		...
		users.forEach((u) => {
			const statistic = {
				...
				Telefoonnummer: u.phoneNumber
					? u.phoneNumber.replace(',', '.')
					: 'Onbekend',
				...
				Nationaliteit: u.nationality
					? u.nationality.replace(',', '.')
					: 'Onbekend',
				Moedertaal: u.nativeLanguage
					? u.nativeLanguage.replace(',', '.')
					: 'Onbekend',
				NLtaalniveau: u.dutchProficiency
					? u.dutchProficiency.replace(',', '.')
					: 'Onbekend',

				PushBerichten: u.pushDisabled ? 'Uitgeschakeld' : 'Ingeschakeld',
				DatumAkkoordPrivacyPolicy: u.termsOfUseConsented
					? DateConverter.toReadableStringFromString(u.termsOfUseConsented)
					: 'Onbekend',
			};
			statistics.push(statistic);
		});
