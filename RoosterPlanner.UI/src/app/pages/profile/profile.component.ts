...
export class ProfileComponent implements OnInit {
	...
	editRemark(id: string) {
		...
		const textAreaId =
			id === 'personalbutton' ? 'personalremark' : 'staffremark';
		const originalText =
			id === 'personalbutton'
				? this.user.personalRemark
				: this.user.staffRemark;
		...
		if (element?.innerText === 'Aanpassen') {
			...
			if (textareaElement.value !== originalText) {
				id === 'personalbutton'
					? (this.user.personalRemark = textareaElement.value)
					: (this.user.staffRemark = textareaElement.value);
				this.userService.updatePerson(this.user).then((res) => {
					if (res) this.loadUserProfile();
				});
			}
		}
	}
	...
	expandCertificateCard() {
		...
		if (this.certificateStyle === 'expanded-card') {
			...
			if (rightElement) rightElement.hidden = false;
			if (remarkElement) remarkElement.hidden = false;
			if (expendedCardElement) expendedCardElement.hidden = true;
			...
			if (rightElement) rightElement.hidden = true;
			if (remarkElement) remarkElement.hidden = true;
			if (expendedCardElement) expendedCardElement.hidden = false;
