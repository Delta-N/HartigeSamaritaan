...
export class AllTasksComponent implements OnInit {
	...
	constructor(
		...
		const array: Breadcrumb[] = [
			...
			this.breadcrumbService.admincrumb,
			takencrumb,
		];
	...
	async getCertificates(offset: number, pageSize: number) {
		await this.certificiateService.getAllCertificateTypes().then((res) => {
			if (res) this.certificatesTypes = res;
	...
	expandCategoryCard() {
		...
		if (this.categoryCardStyle == 'expanded-card') {
			...
			if (taskCardElement) taskCardElement.hidden = true;
			if (certificateCardElement) certificateCardElement.hidden = true;
	...
	expandCertificateCard() {
		...
		if (this.certificateCardStyle == 'expanded-card') {
			...
			this.itemsPerCard = 5;
			this.certificatesTypes = this.certificatesTypes.slice(
				0,
				this.itemsPerCard
			);
		} else {
