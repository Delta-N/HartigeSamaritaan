...
export class EmployeeComponent implements OnInit {
	...
	async ngOnInit(): Promise<void> {
		...
		if (this.guid)
			...
			await this.userService.getAllUsers().then((res) => {
				if (res) this.dataSource = new MatTableDataSource<User>(res);
				this.loaded = true;
			});
		...
		this.dataSource.filterPredicate = (data, filter) => {
			return (
				...
				(data.nationality &&
					data.nationality.toLocaleLowerCase().includes(filter)) ||
				DateConverter.calculateAge(data.dateOfBirth)
					.toLocaleLowerCase()
					.includes(filter)
			);
		};
	...
	details(id: string) {
		this.guid
			? this.router.navigate(['manage/profile', id]).then()
			: this.router.navigate(['admin/profile', id]).then();
	}
