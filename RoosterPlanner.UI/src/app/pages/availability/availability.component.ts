...
export class AvailabilityComponent implements OnInit, AfterViewInit {
    ...
    this.route.paramMap.subscribe(async (params: ParamMap) => {
      ...
      await this.participationService.getParticipation(this.userId, this.projectId)
        .then(async res => {
          ...
          // create breadcrumbs
          const current: Breadcrumb = new Breadcrumb('Beschikbaarheid opgeven', null);
          const previous: Breadcrumb = new Breadcrumb(this.participation.project.name, '/project/' + this.participation.project.id);
  ...
  dateChanged() {
    ...
    if (window.innerWidth <= 768) {
      const element: HTMLCollectionOf<Element> = document.getElementsByClassName('calendarbox');
  ...
  openInstructions(id: string | number) {
    const url: string = this.shifts.find(s => s.id === id).task?.instruction?.documentUri;
    if (url) {
  ...
  changeLoadedShiftBordersAndStar() {
    this.shifts.forEach(s => {
      if (s.availabilities && s.availabilities.length > 0) {
        ...
        const action: string = this.getAvailabilityTypeActionName(availability.type);
  ...
  hideActionButton(id: string) {
    const element = this.getActionElement(this.allEvents.find(e => e.id === id));
    if (element) {
