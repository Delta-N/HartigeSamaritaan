...
export class AvailabilityComponent implements OnInit, AfterViewInit {
	...
	async ngOnInit(): Promise<void> {
		...
		this.route.paramMap.subscribe(async (params: ParamMap) => {
			...
			await this.availabilityService
				...
				.then((res) => {
					if (res) this.availabilityData = res;
	...
	async handleEvent(action: string, event: CalendarEvent): Promise<void> {
		...
		else if (shift) shift.availabilities = [];
		...
		if (availability && availability.type !== 3) {
			...
			await this.availabilityService
				...
				.then((res) => {
					if (res) {
						availability = res;
						if (shift) shift.availabilities[0] = res;
					}
					this.apiCall = false;
				});
			this.changeColor();
	...
	openInstructions(id: string | number) {
		const url: string | null | undefined = this.shifts?.find((s) => s.id === id)
			?.task?.instruction?.documentUri;
	...
	addEvents() {
		...
		this.shifts?.forEach((s) => {
			...
			if (scheduled) {
				...
				if (event.color) event.color.primary = '#5b5bdc';
			}
