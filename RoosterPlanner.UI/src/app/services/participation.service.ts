	...
	async postParticipation(
		...
		if (!participation.project || !participation.person) {
			this.errorService.error(
				'Ongeldige participation in participationservice'
			);
			return null;
		}
	...
	async updateParticipation(
		...
		try {
			if (participation.project.participationEndDate) {
				participation.project.participationEndDate = DateConverter.toDate(
					participation.project.participationEndDate
				);
			}
			participation.project.participationStartDate = DateConverter.toDate(
				participation.project.participationStartDate
			);
			participation.project.projectEndDate = DateConverter.toDate(
				participation.project.projectEndDate
			);
