...
export class ApiService {
	...
	get<T>(url: string, options?: Options) {
		return this.http
			.get<T>(url, this.createHttpOptions(options))
	...
	post<T>(url: string, body?: any, options?: Options): Observable<T> {
		return this.http
			.post<T>(url, body, this.createHttpOptions(options))
			.pipe(map(this.onSuccessResponse), catchError(this.onErrorResponse));
	}
	 ...
	 * @param options the http options such as headers and params
	 */
	postMultiPartFormData<T>(
		url: string,
		body?: any,
		options?: Options
	): Observable<T> {
		return this.http
			.post<T>(url, body, this.createHttpOptionsMultiPartFormData(options))
			.pipe(map(this.onSuccessResponse), catchError(this.onErrorResponse));
	}
	...
	put<T>(url: string, body?: any, options?: Options): Observable<T> {
		return this.http
			.put<T>(url, body, this.createHttpOptions(options))
			.pipe(map(this.onSuccessResponse), catchError(this.onErrorResponse));
	}
	...
	patch<T>(url: string, body?: any, options?: Options): Observable<T> {
		return this.http
			.patch<T>(url, body, this.createHttpOptions(options))
			.pipe(map(this.onSuccessResponse), catchError(this.onErrorResponse));
	}
	...
	delete<T>(url: string, options?: Options): Observable<T> {
		return this.http
			.delete<T>(url, this.createHttpOptions(options))
			.pipe(map(this.onSuccessResponse), catchError(this.onErrorResponse));
	}
	...
	private createHttpOptions(options: Options = {}): Options {
		const { headers = {}, params, observe = 'response' as const } = options;
		return {
			headers: {
