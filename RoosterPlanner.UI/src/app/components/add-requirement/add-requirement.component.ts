		...
		this.certificateService.getAllCertificateTypes().then((res) => {
			if (res) {
				...
				if (this.requirement?.certificateType)
					this.certificateTypeControl.setValue(
						this.allCertificateTypes.find(
							(c) => c.name === this.requirement.certificateType.name
						)
					);
			}
		});
		...
		if (this.checkoutForm.status === 'INVALID') {
			...
			if (this.modifier === 'toevoegen') {
				await this.requirementService
					.postRequirement(this.updatedRequirement)
					.then(async (response) => {
				...
				await this.requirementService
					.updateRequirement(this.updatedRequirement)
					.then(async (response) => {
						this.dialogRef.close(response);
					});
			}
